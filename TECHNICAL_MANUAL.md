...
* `PHSYIO_ID`s from `physregs_layer` in which a `naipqq_layer` polygon is
  contained are out put into this field.
  * Output format: (,1,2,...,)
...
* This input/output raster is used to snap NAIP tiles to a consistent grid
  ...
  that `canopy.reproject_input_tiles()` can automatically create it based on
...
**NOTE:** `physregs_layer` and `naipqq_layer` must be added to open ArcMap or
ArcGIS Pro dataframe for function to run.
...
* Process:
  ...
  2. If the snap raster does not exist within the `snaprast_path` then it is
     created and `arcpy.env.snapRaster` is used to set all output cell
  ...
  5. The outputs of this function are saved in an inputs folder and are what
     will used by Textron's Feature Analysis.
...
* Process:
  ...
  3. Conversion necessary as some AFE models used in feature analysis output
	...
	1 and 0.
     2. if the file is a TIFF file is the values are reclassified from 1 to 0
	and 2 to 1.
     3. If the file has already run through this function and has the
	appropriate prefix then nothing happens to it.
...
* This function mosaics clipped final TIFF and then clips the mosaicked files
...
* Process:
  ...
  3. Input tiles to be mosiacked are products from `canopy.clip_final_tiles`
     with the prefix 'cfr'.
...
* Parameters:
  * `phyreg_ids` (`str`): IDs of physical regions to process
  * `analysis_years` (`int`): Years to add as fields
...
* Process:
  ...
  3. Fields are created in each point shapefile with the header of
     'GT_`analysis_years`'
  4. The values of each classified cell that contains the randomized points
     will be read. This will be done using a NumPy.
